...
mod tests {
    ...
    try_parse_fail!(meta_value_confused_as_list, MetaValueTest, { test(3u8, 3u8) });
    ...
    fn meta_value_keyword_token() {
        ...
        assert_eq!(format!("{:?}", keyword.span()), format!("{:?}", value.keyword_span()));
    }
    ...
    try_parse_fail!(meta_list_wrong_delimiter, MetaListTest, { test_list = (3u8, 3u8) });
    ...
    try_parse_fail!(meta_attr_list_confused_as_list, MetaAttrListTest, { wrong(i32) });
